/* 
Implementing GridLayout instead of manually setting the buttons
1) Create a hashmap and have the button name as the first value and the second value be the new JButton. 
2) Create a panel. 
3) Create an array that has the values that the buttons are going to be displayed at. 
4) Loop through the array and set the second value of the hashmap equal to new JButton(buttons[i]). Add the new JButton to the panel. 
problem: how do you check for when each button is clicked? By getting the value of the key in hashmap? But is new JButton(value) the button itself or is it just going to create a new button? Since you can't create variables dynamically, how do I check if e.getSource() is equal to the button? 
*/
